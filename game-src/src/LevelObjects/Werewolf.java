...
/**
 * Class represents an enemy(Werewolf) with its graphics and properties
...
public class Werewolf extends NPC {
	...
	/*
	 * Werewolf spezifische Eigenschaften
	 * Die Standardwerte für diese Eigenschaften
	 */
    private static int STANDARD_CAPTURERADIUS = 5*5;
    protected static byte STANDARD_SPEED = 3;
    protected static byte STANDARD_LIFEPOINTS = 16;
    protected static short STANDARD_ATTACKDAMAGE = 2;
    
    protected static int STANDARD_ATTACKTIME = 10;
    protected static byte STANDARD_MOVETIME = 4;
	protected static short STANDARD_ATTACKDURATION = 5;
    
	/**
	 * Constructor for the NPC class Werewolf
	 * @param id
	 * @param position
	 * @param parentBlock
	 */
	public Werewolf(short id, Position position, Block parentBlock, boolean disabled, int npcType, byte itemWhenDying) {
		super(id, position, parentBlock, disabled, npcType, itemWhenDying);
		
		setEnabled(!disabled);
		
		this.setPosition(position);
		setCaptureRadius(STANDARD_CAPTURERADIUS); // set the capture radius of the zombie
		setMoveDirection(Level.dirNone); //set the movedirection
	    ...
	    this.setAttackTime(STANDARD_ATTACKTIME); // Set the time to start an attack - StandardDauer 2
	    this.life = new LifeStatus(STANDARD_LIFEPOINTS);
	    this.invStunPossibility = 2; //Sets  the possibilty to stun the enemy
		...
		try {
			...
			new Error("werwolf sprite init", e);
		}
		 ...
		 if (this.getLifePoints()>0) {//			 tests if the enemy is alive
		  if (this.getStunDuration()<=0) { // tests if the enemy is not stunned 
			  if (this.getUnmoveableFrames()<=0){ // tests if the enemy is not frozen
				...
				if (capture()){ // if frozen the enemy can still attack but not move
					this.life.setEnabled(true); // Activates the lifestatus
					if (inDistanceToPlayer()){
						attack();
					}
				}
		       this.decreaseUnmoveableFrames(UPDATE_FREQUENCY); // decrease the time of frozen
			  }
		  }
		  else {
    ...
    */
	...
	protected byte getFrameDuration(byte animation, byte frame) {
		if( animation >= ANIM_UP && animation <= ANIM_LEFT ) {
			return 1;
		}
		return UPDATE_FREQUENCY;
	}
	...
	/*
	 * Autmatisierter Test der Klasse Werewolf
